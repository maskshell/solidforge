...
A2 (OPERATOR decision) → A6 (OPERATOR-executed human gate) → A3 → B1 → A4+§C-C1 → A5 → D1 → (B2, B3 optional) → (§C-C2..C7 next-use practices; D3 blocked on D1 (D2 unblocked); §E C1–C9 trigger-gated per the backlog; §F folded at their own touch-points (F4 next-bc; F5 Phase-B decision, outcome-axis)
...
- **A2 — Paper pub-readiness-F5 framing decision** (OPERATOR-side; the framing choice is outcome-axis per pub-readiness's own annotation): the title "Specification Gaming as an Orthogonal Failure Axis in Autonomous Coding Loops: The Verification-Source Decoupling Discipline" does not flag the stipulated (not established) self-certification conjecture that the orthogonality claim rests on. Options per pub-readiness-F5 — which addresses TWO carriers (the title is silent on the stipulation AND the Abstract's opening asserts precision before the mid-Abstract stipulation clause): (a) foreground the stipulation clause in the Abstract's opening; (b) soften "orthogonal" to "orthogonal under the conjecture" in title AND body. Precedent note: pub-readiness observes the stipulation clause ('under the self-certification conjecture of §3.1, stipulated rather than established') sits at Abstract L17 of the CURRENT canonical (THIS DOC's re-verification, 2026-09-16; pub-readiness's own coordinate is the pre-fix L20 — the frontmatter slimming shifted it) — option (a) is the smaller delta (echo the clause into the opening sentences). Locus: edit the CANONICAL (KB), then sync-paper.sh.
- **A3 — Paper §6.x incorporation**: docs/papers/retrospective-section-draft.md (numbers final: 23 positives of 667 pool / 319 candidates; 3.4%/7.2%; 96.9% strict-match; five of ten units) slots into the canonical paper's §6 after the instantiation paragraphs — CONDITIONAL on A6 (the numbers are pipeline-final; the human spot-check is the design's finality gate and may still adjust RQ2, per the results doc's own draft status). Canonical lives in the author KB (P4-KB/docs/papers/spec-gaming-orthogonal-axis.md); snapshot discipline: edit canonical, then run sync-paper.sh; PDF must be regenerated from .tex if older than the .md.
- **A4 — psv escalation arms adopt/reject** (protocol next-revision gate): decide each arm — K>0 (unverifiable claims present) and M=0 (no admissible surface) — for inclusion in the protocol's rule 1.
- **A5 — Evidence-corpus placement decision**: three options — remain untracked (current, structural via .gitignore), scrub-and-commit, or move outside the repo. The design's scrub norms said the decision is settled by the first analysis output; that output now exists. State: the committed results/draft docs are pseudonymized at source (P1-P4); the analysis/ scripts and rq2 records remain UNTRACKED in the corpus pending this decision; the raw tree carries real consumer names.

- **A6 — Design deliverable-3 human spot-check** (gates the results doc's finality): the design's human step remains PENDING (orchestrator-tier deviation disclosed, never superseded — protocol header row 6). Until a human reviews the RQ2 positives sample, docs/papers/retrospective-eval-results.md stays 'draft — spot-check may adjust RQ2'. Operator-side; a 5-minute read of docs/papers/evidence-corpus/analysis/rq2-coding-result.json positives (23 items, each with a ≤12-word quote).
...
- **B1 — References psv re-run or record attachment**: pub-readiness-F6 requires the References list's citation grounding to be a fresh psv run or an attached record of the 2026-07-31 run. Runnable here: solidforge:primary-source-verification over the CANONICAL paper's References (KB copy; if verifying from the snapshot, run sync-paper.sh first — edits land in the canonical only, sync is canonical→snapshot and clobbers the destination).
- **B2 — Osmani-2026 blog as the "loop engineering" primary citation**: judgment call (swap for an academic source or keep and label).
...
- **§C-C1 — Protocol worked instance**: a walk-through with DESIGN PARAMETERS 40 items / 2 units / 4 positives / 1 seat / 8-item subsample (the 20% ratio applied; numbers are this instance's design, not persisted from any prior execution — the record labels the narration's substance unpersisted), one line per rule per draw class, replacing the UNREVIEWED NARRATION label (the debt was raised by the round-12/13 review findings R12-9/R13-3 ('round-8 walk-through substance unpersisted')).

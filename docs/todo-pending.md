...
A2 (OPERATOR decision) → A6 (OPERATOR-executed human gate) → A3 → B1 → ~~A1-A5/B1/B2/D1/D2/D3~~   [ALL DONE 2026-09-16; remaining: §C-C2..C7 (next-use practices, automatic on next calibration cycle); B3 (optional, advisory); B2 resolved (Osmani verified + kept); paper line: position-paper tier CLEARED for submission] → (B2, B3 optional) → (§C-C2..C7 next-use practices; D3 blocked on D1 (D2 unblocked); §E C1–C9 trigger-gated per the backlog; §F folded at their own touch-points (F4 next-bc; F5 Phase-B decision, outcome-axis)
...
- **A1 — KB-side readiness addendum commit**: DONE 2026-09-16 — committed in the KB repo as part of the paper-edit series (d313661 → aec8379, 6 commits total: fix-status addendum, F5 stipulation, §6.5, References fixes, Unicode fixes, D3 Limitations). The KB repo has NO remote configured — push remains operator-side.
...
- **D2 — RQ3 fine-grained drip**: DONE 2026-09-16 — kind × file/§ cluster: 131 clusters, 41 multi-instance, 38/41 (92%) spanning >1 round; the C9 drip shape confirmed at fine grain (top: structural-gap × agent.md 9 instances across 7 rounds). Results doc updated.
- **D3 — RQ2 per-leg skew retest**: DONE 2026-09-16 — 9/12 nulls resolved via multi-method (leg-files + D1 pairs + ID-naming + peer-overlap); result: same 17 / hetero 3 / null 3 — the design's skew expectation NOT met (skew toward the fix-authoring same-family leg, not away). Hetero's value is discovery complementarity, not fix-residue detection.

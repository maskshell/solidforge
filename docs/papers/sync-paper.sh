...
# Copies the paper artifact set VERBATIM (no transforms) and reports freshness.
# The snapshot must NEVER drift silently: run this only as a deliberate act.
# (Mirrors solidforge-dsh's scripts/sync-paper.sh — same FILES set; this copy
#  lives beside the snapshot it manages, so DEST is the script's own dir.)
set -euo pipefail

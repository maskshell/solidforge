...
> v2 (2026-08-27), via the Revision Channel: v1's AC-4 named a "SKILL.md/install.md
> self-check lists" pair, but install.md carries NO gate list (it is a provisioning
> doc; none of the 7 existing gates are enumerated there) — satisfying it literally
> would CREATE a redundant, drift-prone enumeration. Amended to the enumerations that
> actually exist. v1 remains at csr-run-progress-v1.blueprint.md as frozen history.
...
- AC-4: Given the shipped state, When `csr_progress_gates.py` runs, Then the registry
  ...
  wrapper divergence is recorded in `hetero_doc_review.divergence.md`,
  `disconnect_check.py` REQUIRED_FILES includes both new files, the SKILL.md
  self-check list includes the new gate, and install.md's observability section
  mentions `csr_progress.py` — any drift is a Blocker. — seam: `csr_progress_gates.py`
  gate (catches enumeration drift; misses prose quality — doc-audit advisory, rule 5)

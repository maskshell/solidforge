...
| C6 | memory 信心分层 | E4（`workflow-from-chats`） | 登记项 | strong/medium/weak/contradicted 四档；`contradicted` → 写入前先问用户（预期价值低） | 跨会话 memory 冲突复发；外部设计点（Teknium 2026，2026-09-17 fetch——nousresearch.com/refactoring-hermes-with-1393-agents）：Hermes 的 skill 在工作中自动积累（"Hermes (automatically) noticed this and saved the reusable lesson"），人类纠正后由 agent 维护——与本工作区 ADR+dogfood+自门纪律互为反极；按论文 §4.1 authorship-collapse 规则，agent 自写的 skill 是 same-source oracle、盲点会遗传——其可用性证据与我们纪律的成本共同喂给 C6 分层设计（哪档信号挣得 auto-write：人类纠正=strong；agent 自察=至多 weak） |

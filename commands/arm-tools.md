...
- L1 Constitution + Gate-Toolchain sections appended to the project `CLAUDE.md` (once; idempotent).
- intent-blueprint template + cold-start-patterns copied to `docs/intent-blueprints/_templates/`.
- `.gitignore` entries added for the convergence-loop runtime state (`.claude/parallel-dev/loop-state.json`, `.claude/parallel-dev/runs/`).
...
## Step 3 — LSP / code-intelligence advisory (do NOT install yourself)

Solid Forge does NOT bundle `.lsp.json` and does NOT install language servers. For each detected language, recommend the matching **official LSP plugin** from `claude-plugins-official` and the language-server **binary** install command (the binary must be on `$PATH`):

- Python: install plugin `pyright-lsp`; binary `npm install -g pyright` (or `pipx install pyright`).
- Rust: install plugin `rust-analyzer-lsp`; binary `rustup component add rust-analyzer`.
- Swift: install plugin `swift-lsp`; binary ships with Xcode/`xcrun sourcekit-lsp`.
- Java: install plugin `jdtls-lsp`; binary — download Eclipse JDT.LS or `brew install jdtls`.
- TypeScript/Web: install plugin `typescript-lsp`; binary `npm install -g typescript typescript-language-server`.

State clearly that the official LSP plugins own the Claude↔LSP wiring + auto-diagnostics, and that installing all of them is optional — opt in per language this project uses.

## Step 4 — report

Tell the user concisely: what was armed (configs, deps if `--with-tools`, constitution, templates, gitignore, and the `.env.solidforge.example` secrets placeholder), which gates are still absent, and the LSP recommendation(s) for this project's languages. Note that the Solid Forge hooks are already active (Layer 1) and that `arm.py --revert` (dry-run; add `--apply` to execute) removes only the template-matching provisioned files, preserving any user edits.

...
- **Dual-/multi-different-family** (extensibility): `--profile deepseek,qwen3` runs each backend independently and merges findings (each finding tagged with its `provider` for N-way reconciliation). Pick two providers NEITHER of which is the orchestrator's primary — e.g. in a GLM-orchestrated project, `deepseek` + `qwen3` (BigModel is same-family there, so it is not a true different-family in that project).

...
| --- | --- | --- | --- |
| Fast gate (`fast_gate.py`) | Every Edit/Write to an in-repo file (out-of-repo scratch = silent no-op, ADR #59) | PostToolUse hook; per-file lint (Blocker, fix-in-ring) + format (Blocker, commit-stratified remediation — [commit-stratification.md](commit-stratification.md)); format-emitting checks: Python/Java/Go/Rust; Swift/Web/Python are lint | `decision:block` + reason; Claude self-corrects next turn; inner short-circuit |
| Blueprint guard (`blueprint_guard.py`) | Edit/Write to a frozen blueprint | PreToolUse hook | `permissionDecision:deny`; edit blocked — EXCEPT the append-only AC→test mapping carve-out (ADR #58): append bullets to the mapping section passes, everything else (incl. bullet delete/rename) denies |

...
def _resolve(name, root):
    ...
    This gate resolves NO tool project-locally on its own (2026-09-06,
    node-bin adoption A3). Lazy import inside the function mirrors the lazy-import idiom this change-set
    standardizes (E402-clean at module level)."""
    lib = os.path.join(
...
def depcruise_cmd(root):
    ...
    # resolution, so this arm rides only under the node opt-in (2026-09-06,
    # node-bin adoption A3). Outside the opt-in the gate degrades honestly.
...
def check_eslint_concurrency(root, findings, coverage, src):
    ...
    if not tool:
        coverage.append(
            "eslint: not installed — sync-in-async concurrency check skipped (install: `npm i -D eslint`, opt in with SF_PROJECT_NODE_BIN=1 for project-local)"
        )
        return
    rc, out = run(
...
def _resolve_tsc(root):
    ...
    the version-coupling trade-off recorded in design-decisions.md ADR #64)."""
...
def check_types(root, findings, coverage, src):
    ...
    if not tsc:
        coverage.append(
            "tsc: not resolved — type-check skipped (PATH, or install + `SF_PROJECT_NODE_BIN=1` for the project-local copy)"
        )
        return

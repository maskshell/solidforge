...
NOTE (v2.1.238 re-probe, 2026-08-21 — three drifts, ADR #52):
  ...
  2. Alias remap VERIFIED end-to-end through the substrate's own materialization
...
def _claude_argv(
    ...
    are ADDITIVE: pre-ADR-#52 positional callers stay source-compatible (csr's
    divergence.md contract row updates in lockstep — rule 7)."""
    argv = [
        "claude",
        "-p",
        "--settings",
...
def _run_claude_once(
    ...
    DEGRADE logic preserved (ADR #41). The trailing `guards` kwarg is ADDITIVE
    (None = legacy json spawn via subprocess.run; a dict {provider,
    max_stream_bytes} = stream spawn via _run_streamed — heartbeat + byte cap +
    telemetry, ADR #52), so positional callers stay source-compatible.
    Ported from csr's hetero_doc_review.py (ADR #52 — rule 7 lockstep).
    """

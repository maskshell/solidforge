...
Covers the project-local tool resolution contract on the CC substrate
(2026-09-06 node-bin adoption; docs/sf-project-node-bin-adoption.md; ADRs
#63/#64/#65 — ported from pi 0.2.7's detect_toolchain_test.py, extended with
the seven collapsed-site probes this adoption added):
  ...
  1. PATH-only by default — a repo-committed node_modules/.bin/<tool> (or
     ...
     execute-repo-committed-binaries class; SF_PROJECT_VENV_TOOLS landed with
     489b217, SF_PROJECT_NODE_BIN with this adoption).
  ...
  4. SF_PROJECT_VENV_TOOLS regression (489b217 behavior unchanged).
  5. arm.py tool_present follows the same trust model (opt-in-gated,
     root-threaded — no silent-green, no cwd≠target drift).
  6. CC sites: the SEVEN formerly ad-hoc resolution sites (fast_gate eslint;
     ...
     SF_PROJECT_NODE_BIN (a PATH tool is not PATH resolution — ADR #65).

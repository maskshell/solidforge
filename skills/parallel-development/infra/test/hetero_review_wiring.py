...
def check_provider_template_expansion():
    ...
    # materialize deepseek with the CONVENTION var → token injected, routing preserved
    os.environ["DEEPSEEK_ANTHROPIC_AUTH_TOKEN"] = "sk-conv-unit"
    tmp = h._materialize_profile("deepseek")
    try:
        d = json.load(open(tmp))
        assert d["env"]["ANTHROPIC_AUTH_TOKEN"] == "sk-conv-unit", d["env"]
        assert d["env"]["ANTHROPIC_BASE_URL"] == "https://api.deepseek.com/anthropic"
    finally:
        os.unlink(tmp)

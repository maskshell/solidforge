...
3. Convergent Fix Loop — immediately enter this loop after merging. Do not report partial results or wait for user input between iterations. Loop autonomously until convergence:
   ...
   Spawn an independent `solidforge:code-reviewer` subagent to review the final Diff against the Intent Blueprint and the L1 Constitution. Review all changed files for:
...
- Per-task `files_touched`: List of file paths the task will read or modify, stored in TaskCreate metadata. Used for conflict detection.

### Scheduling Loop

Claude's execution model is turn-batched: multiple Agent tool calls issued in one message run in parallel and return results together. The scheduling loop operates per-turn:

```markdown
1. Identify all subtasks and their `files_touched`.
2. Create a TaskCreate for each subtask (status: pending).
3. While pending or running tasks remain:
   ...
   b. For each pending task (in priority order):
      ...
      - Otherwise: launch Agent, set task status to in_progress, decrement available_slots.
   c. Wait for running agents to return results.
...
```text
TaskCreate: Write tests for AppHeader → files: [AppHeader.ts, AppHeader.test.ts]
TaskCreate: Write tests for AppSidebar → files: [AppSidebar.ts, AppSidebar.test.ts]
TaskCreate: Write tests for AppMain → files: [AppMain.ts, AppMain.test.ts]
```

GREEN phase (scheduled -- no file overlaps, all can run in parallel):

```text
TaskCreate: Implement AppHeader → files: [AppHeader.ts], agent: solidforge:frontend-developer
TaskCreate: Implement AppSidebar → files: [AppSidebar.ts], agent: solidforge:frontend-developer
TaskCreate: Implement AppMain → files: [AppMain.ts], agent: solidforge:frontend-developer
```
...
```text
TaskCreate: Write API tests → files: [auth.api.test.ts]
TaskCreate: Write UI tests → files: [login.ui.test.ts]
```

GREEN phase (backend and frontend are independent):

```text
TaskCreate: Create API → files: [src/auth/], agent: solidforge:backend-developer
TaskCreate: Create UI → files: [src/components/Login.vue], agent: solidforge:frontend-developer
```
...
```text
TaskCreate: Write tests for LoginViewModel → files: [LoginViewModel.swift, LoginViewModelTests.swift], prompt: "Swift Testing @Test for @Observable ViewModel"
TaskCreate: Write tests for UserService → files: [UserService.swift, UserServiceTests.swift], prompt: "Swift Testing @Test for actor service"
TaskCreate: Write XCUITest for login flow → files: [LoginView.swift, LoginUITests.swift], prompt: "XCUITest with accessibility identifiers"
```

GREEN phase (scheduled — independent SwiftUI views/components):

```text
TaskCreate: Implement LoginViewModel → files: [LoginViewModel.swift], agent: ios-developer (prompt: "iOS SwiftUI developer — implement @Observable LoginViewModel with @MainActor")
TaskCreate: Implement LoginView → files: [LoginView.swift], agent: ios-developer (prompt: "iOS SwiftUI developer — implement LoginView using @Observable ViewModel")
TaskCreate: Implement UserService actor → files: [UserService.swift], agent: ios-developer (prompt: "iOS Swift developer, Swift Concurrency expert — implement Sendable-safe UserService actor")
```
...
```text
TaskCreate: Write tests for user router → files: [app/api/v1/users.py, tests/test_api/test_users.py], prompt: "pytest + FastAPI TestClient, test CRUD endpoints"
TaskCreate: Write tests for order router → files: [app/api/v1/orders.py, tests/test_api/test_orders.py], prompt: "pytest + FastAPI TestClient, test CRUD endpoints"
```

GREEN phase (scheduled — independent routers, no shared files):

```text
TaskCreate: Implement user router + schemas → files: [app/api/v1/users.py, app/schemas/user.py], agent: solidforge:backend-developer
TaskCreate: Implement order router + schemas → files: [app/api/v1/orders.py, app/schemas/order.py], agent: solidforge:backend-developer
```
...
- [host-routing.md](references/host-routing.md) - Optional host-project CLAUDE.md routing snippet surfaced by /solidforge:arm-tools (bc/pd/csr self-route via Scope Guards; the csr explicit-invocation gap)
